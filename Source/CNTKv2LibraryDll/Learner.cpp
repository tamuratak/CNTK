...
namespace CNTK
{
    ...
    LearnerPtr AdamLearner(const vector<Parameter>& parameters,
                           ...
                           AdditionalLearningOptions additionalOptions /*= AdditionalLearningOptions()*/)
    {
        // TODO: This is due to history reason of the legacy AdamLearner using FSAdaGrad implmentation instead of the original paper implementation.
        //      To keep interface backward compitability, we will keep the legacy logic path by default and switch to the original adam only when lowMemory is false.
        if (!lowMemory)
        {
